...
extern struct command mail_list[], news_list[], snews_list[];
...
void            exit_snews_mode(player * p, char *str)
{
   if (p->input_to_fn != snews_command)
      return;
   tell_player(p, " Leaving SU news mode.\n");
   p->input_to_fn = 0;
   p->flags |= PROMPT;
   p->mode &= ~SUNEWSEDIT;
}
   ...
   tell_player(p, " Article posted....\n");
   {
	for (scan = flatlist_start; scan; scan = scan->flat_next)
		if (scan->custom_flags & NEWS_INFORM && scan != p)
			...
			TELLPLAYER(scan, " -=*> A new news article, '%s', has been posted anonymously.\n", article->header);
		     else
			TELLPLAYER(scan, " -=*> A new news article, '%s', has been posted by %s.\n", article->header, article->name);
   }
   p->mode &= ~NEWSEDIT;
...
void            post_news(player * p, char *str)
{
   ...
   article = create_note();
   strncpy(article->header, str, MAX_TITLE - 3);
...
void            followup(player * p, char *str)
{
   ...
   else
   {
      ...
      strncpy(article->header, stack, MAX_TITLE - 3);
   }
   article->flags |= NEWS_ARTICLE;
...
void 		give_news_notimeout(player * p, char *str)
{
	char *oldstack;
	note *article;

	if (!*str || !isdigit(*str)) {
		tell_player(p, " Format: nto <article-number>\n");
		return;
	}
...
void            read_article(player * p, char *str)
{
   ...
   stack = strchr(stack, 0);
   if (p->residency & PSU && article->flags & NEWS_NO_TIMEOUT)
	{
	strcpy(stack, "Article has the no-timeout flag set.\n");
	stack = strchr(stack, 0);
	}
...
void            end_mail(player * p, char *str)
{
    ...
    *stack++ = 0;
      ...
      else sprintf(stack, " Sending mail to %s.\n", name_list);
      stack = end_string(stack);
      tell_player(p, body);
      stack = body;
...
{
   note           *mail;
   char           *subject, *scan, *first, *oldstack;
   int             length, fr=0;
   list_ent	  *l;
...
void            recount_snews(player * p, char *str)
{
   int             article;
   note           *scan;
   snews_count = 0;
   ...
   {
      article = scan->next_sent;
      for (; scan; article = scan->next_sent, snews_count++)
      {
    scan = find_note(article);
    if (!scan)
       break;
...
void            snews_command(player * p, char *str)
{
   ...
   if (!*str)
    ...
    return;
      ...
      {
    tell_player(p, " Entering super user news mode. Use 'end' to leave.\n"
           " '/<command>' does normal commands.\n");
    p->flags &= ~PROMPT;
...
void            end_spost(player * p, char *str)
{
   note           *article;
   char           *oldstack;
   oldstack = stack;
   ...
   article->next_sent = snews_start;
   snews_start = article->id;
   snews_count++;
   article->flags &= ~NOT_READY;
   article->read_count = 0;
   tell_player(p, " Article posted....\n");
   SUWALL(" -=*> New SU News article, \"%s\", posted by %s\n",
		article->header, p->name); 
   p->mode &= ~SUNEWSEDIT;
   if (p->edit_info->input_copy == snews_command)
   {
      do_prompt(p, "SU News >");
      p->mode |= SUNEWSEDIT;
   }
...
void            sfollowup(player * p, char *str)
{
   ...
   if (!old)
   {
      sprintf(stack, " No such su news article '%s'\n", str);
      stack = end_string(stack);
      tell_player(p, oldstack);
      stack = oldstack;
      return;
...
void 		give_snews_notimeout(player * p, char *str)
{
	char *oldstack;
	note *article;
   ...
   if (article->flags & ANONYMOUS)
   {
    ...
    sprintf(stack, "Subject: %s\nPosted anonymously on %s.\n",
      ...
      sprintf(stack, "Subject: %s\nPosted by %s on %s.\n", article->header,
         article->name, convert_time(article->date));
   stack = strchr(stack, 0);
   if (article->flags & NEWS_NO_TIMEOUT)
	{
	strcpy(stack, "Article has the no-timeout flag set.\n");
	stack = strchr(stack, 0);
	}
...
/* reply to an su article */

void            reply_suarticle(player * p, char *str)
{
   note           *mail, *old;
   char           *indent, *body, *oldstack, *scan, *first;
   int             length;
