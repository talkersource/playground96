...
room *create_room(player * p)
{
   ...
   strcpy(r->id, id);
   sprintf(r->name, "in a bare patch of grass owned by %s.", p->name);
   sprintf(r->enter_msg, "goes to %s's unmodified patch.", p->name);
   r->flags = (home * HOME_ROOM)|ROOM_UPDATED|EXITMSGS_OK;
   sprintf(stack, "\n A bare patch of grass, belonging to %s.\n"
...
void            room_soundproof(player * p, char *str)
{
   ...
   if (current_room->flags & SOUNDPROOF) {
      tell_player(p, " You seal the room in think, soundproof plastic.\n");
...
void            room_antising(player * p, char *str)
{
   ...
   if (current_room->flags & ANTISING) {
      tell_player(p, " You chanber the room to prevent singing in this room.\n");
...
{
   tell_player(p, " Just where do you think you are anyway? :P\n");
}
