...
void            construct_name_list_swho(player ** list, int count, int swho)
{
   ...
   /*
    * scan=list; if (swho) { if (current_player->term_width)
    * space=(current_player->term_width/2)-9; else space=39; for
    * (number=space;number;number--) *stack++=' ';
    * sprintf(stack,"%s",full_name(*scan)); stack=strchr(stack,0); for
    * (number=space;number;number--) *stack++=' '; stack++='\n'; scan++;
    * count--; }
    */
   ...
   for (scan = list, n = count; n; n--, scan++)
   {
    ...
    fname = full_name(*scan);
     ...
     if (swho == 2 && ((*scan)->tag_flags & BLOCK_FRIENDS || (*scan)->tag_flags & BLOCK_TELLS))	
	*stack++ = '(';
     else
    	*stack++ = ' ';
...
/*
 * { int max_length=0,n,m,cols,col_count; int len; char *fname; player **scan;
 * if (!count) return;
 * 
 * for(scan=list,n=count;n;n--,scan++) { len=strlen(full_name(*scan)); if
 * (len>max_length) max_length=len; } if (current_player->term_width)
 * cols=(current_player->term_width)/(max_length+3); else
 * cols=78/(max_length+3); col_count=cols;
 * 
 * 
 * for (scan=list,n=count;n;n--,scan++) { if
 * (current_player->saved_flags&NOPREFIX) fname=(*scan)->name; else
 * fname=full_name(*scan); len=strlen(fname); for(m=(max_length-len);m;m--)
 * *stack++=' '; while(*fname) *stack++=*fname++; stack++=' '; stack++=' ';
 * col_count--; if (!col_count) { stack++='\n'; col_count=cols; } }
 * stack++='\n'; }
 */
...
player         *find_player_global(char *name)
{
   ...
   if (strlen(name) < 2) {
	TELLPLAYER(current_player, " No such name - %s -\n", name);
	return 0;
   }
   stack = oldstack;
   lname = stack;
...
char           *your_name_string(player * p, player * tagged)
{
   if (tagged == current_player)
   {
    ...
    strcpy(stack, "your");
      ...
      {
    /*
     * switch (tagged->gender) { case VOID_GENDER: strcpy(stack,"its");
     * break; case OTHER: strcpy(stack,"its"); break; case MALE:
     * strcpy(stack,"his"); break; case FEMALE: strcpy(stack,"her");
     * break; }
     */
    strcpy(stack, "my");
      }
...
void            au_wall_but(player * p, char *str)
{
   ...
   for (scan = flatlist_start; scan; scan = scan->flat_next)
   {
      if (scan->residency & (LOWER_ADMIN|ADMIN|TESTCHAR) && !(scan->flags & BLOCK_SU)
          && scan != p)
...
void informed_who(player * p, char *str)
{
   ...
   text = stack;
   if (count == 1)
      strcpy(stack, " Only one person on your inform list is on the program.\n");
   else
      sprintf(stack, " There are %s people from your inform list on the program.\n",
         number2string(count));
   while (*stack)
      stack++;
   if (!str || *str == '-' || p->custom_flags & NOPREFIX)
...
void friended(player * p, char *str)
{
   ...
   for (g = flatlist_start; g; g->flat_next) 
   for (l = g->saved->list_top; l; l = l->next)
     {
       if (l->flags & FRIEND)
      {
         f = find_player_absolute_quiet(l->name);
         if (f)
	   if (f->saved || (f->saved->list_top))
	   {
	   if (!((sus && f->residency & SU) ||
		 (newbies && f->residency==NON_RESIDENT)))
	     {
   ...
   if (!count)
   {
      tell_player(p, " No one on right now has friended you.\n");
      stack = oldstack;
      return;
   }
   text = stack;
      sprintf(stack, " %s people on have you listed as 'friend'.\n",
         number2string(count));
   while (*stack)
      stack++;
   if (!str || *str == '-' || p->custom_flags & NOPREFIX)
